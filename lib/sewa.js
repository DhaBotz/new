    ...
    setInterval(() => {
        ...
        if (position !== null) {
            ...
            if (_dir[position].status === true){
                dha.sendMessage(_dir[position].id, `Waktu sewa di grup ini sudah habis, bot akan keluar otomatis`, MessageType.text)
                .then(() => {
                    dha.groupLeave(_dir[position].id)
                    .then(() => {
